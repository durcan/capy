...
def LoadRRQ(detnum, min_date = 0 , max_date = numpy.inf, which_dataSet = 'ba',
    ...
    if eventRQ == False:
        if len(rrqs_to_load) > 0:
            RRQ = root2array(PassingFilesToLoad, 'rrqDir/calibzip%0.1d'%detnum, rrqs_to_load)
        else:
            RRQ = root2array(PassingFilesToLoad, 'rrqDir/calibzip%0.1d'%detnum)
        return RRQ
    if eventRQ == True:
        if len(rrqs_to_load) > 0:
            RRQ = root2array(PassingFilesToLoad, 'rrqDir/calibevent', rrqs_to_load)
        else:
...
def LoadRQ(detnum, min_date = 0 , max_date = numpy.inf, which_dataSet = 'ba',
    ...
    if eventRQ == False:
        if len(rqs_to_load) > 0:
            RQ = root2array(PassingFilesToLoad, 'rqDir/zip%0.1d'%detnum, rqs_to_load)
        else:
...
def LoadCut(detnum, min_date = 0 , max_date = numpy.inf, which_dataSet = 'ba',
    ...
    for i in cuts_to_load:
        print "Loading Cut ", i
        AllFiles            = []
        AllFiles            = glob(LoadingData + '/' + i + '/*v53_01*.root')
        AllFiles.sort()
        ...
        if len(cuts_to_load) > 1:
            cuts = rfn.merge_arrays([cuts,root2array(PassingFilesToLoad, 'cutDir/cutzip%0.1d'%detnum, [i])] , flatten = True, usemask = False)
        else:
            print i, detnum
            cuts = root2array(PassingFilesToLoad, 'cutDir/cutzip%0.1d'%detnum, [i])
    ...
    if len(AllFiles) == -11111111:
        for i in cuts_to_load:    
            ...
            for line in AllFiles:
                ...
                if (FileDate >= min_date) and (FileDate <= max_date):
                    PassingFilesToLoad = numpy.append(PassingFilesToLoad, line)
            cuts = numpy.append_fields(cuts, i , root2array(numpy.sort(PassingFilesToLoad), 'cutDir/cutzip%0.1d'%detnum))
            PassingFilesToLoad = []
